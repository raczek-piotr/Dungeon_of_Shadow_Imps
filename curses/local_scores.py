...
def scoreboard_print(w, c):
    try:
        ...
        with open(path + "scores.txt", 'w') as  scores_txt:
            scores_txt.write("5001|CLASSIC|DWARWISH WARRIOR|7245|28|27|-|[]|True\n625|CLASSIC|FUDISH ARCHER|2214|8|8|-|[]|True\n")
...
def scoreboard_append(w, c, p, wins = False):
    ...
    if p["classicgame"]:
        ...
        points = p["xp"]+(p["attack"]*(p["attack_damage"]+1)*p["attack_acc"]*p["attack_hits"])//5+10*(p["lw"]+p["depth"]+p["armor"]+p["intelligence"]) +p["lw"]**2 -23 #spellbook??? -PR-
    c.curs_set(2)
    q = ""
    nick = ""
